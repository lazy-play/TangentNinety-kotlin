...
/**
 * BetterViewDragHelper is a utility class for writing custom ViewGroups. It offers a number
 * of useful operations and state tracking for allowing a user to drag and reposition
 * views within their parent ViewGroup.
 */
public class BetterViewDragHelper {
    private static final String TAG = "BetterViewDragHelper";
    ...
    /**
     * A Callback is used as a communication channel with the BetterViewDragHelper back to the
     * parent view using it. <code>on*</code>methods are invoked on siginficant events and several
     * accessor methods are expected to provide the BetterViewDragHelper with more information
    ...
    public abstract static class Callback {
         ...
         * one of these methods, the BetterViewDragHelper will enter {@link #STATE_SETTLING}
         * and the view capture will not fully end until it comes to a complete stop.
         * If neither of these methods is invoked before <code>onViewReleased</code> returns,
         * the view will stop in place and the BetterViewDragHelper will return to
         * {@link #STATE_IDLE}.</p>
         *
         ...
         * <p>
         * <p>BetterViewDragHelper may call this method multiple times for the same view even if
    ...
    /**
     * Factory method to create a new BetterViewDragHelper.
     *
     * @param forParent Parent view to monitor
     * @param cb        Callback to provide information and receive events
     * @return a new BetterViewDragHelper instance
     */
    public static BetterViewDragHelper create(@NonNull ViewGroup forParent, @NonNull Callback cb) {
        return new BetterViewDragHelper(forParent.getContext(), forParent, cb);
    }
     ...
     * @return a new BetterViewDragHelper instance
     */
    public static BetterViewDragHelper create(@NonNull ViewGroup forParent, float sensitivity,
                                              @NonNull Callback cb) {
        final BetterViewDragHelper helper = create(forParent, cb);
        helper.mTouchSlop = (int) (helper.mTouchSlop * (1 / sensitivity));
        return helper;
    }

    /**
     * Apps should use BetterViewDragHelper.create() to get a new instance.
     ...
     * @param forParent Parent view to monitor
     */
    private BetterViewDragHelper(@NonNull Context context, @NonNull ViewGroup forParent,
                                 @NonNull Callback cb) {
        if (forParent == null) {
    ...
    public void captureChildView(@NonNull View childView, int activePointerId) {
        if (childView.getParent() != mParentView) {
            throw new IllegalArgumentException("captureChildView: parameter must be a descendant "
                    + "of the BetterViewDragHelper's tracked parent view (" + mParentView + ")");
        }
     ...
     * of the BetterViewDragHelper's knowledge).
     * <p>
    ...
    protected boolean canScrollHorizontally(@NonNull View v, boolean checkV, int dx, int x, int y) {
        if (v instanceof ViewGroup) {
            final ViewGroup group = (ViewGroup) v;
            final int scrollX = v.getScrollX();
            final int scrollY = v.getScrollY();
    ...
    public boolean shouldInterceptTouchEvent(@NonNull MotionEvent ev) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_POINTER_DOWN: {
                ...
                // A BetterViewDragHelper can only manipulate one view at a time.
                if (mDragState == STATE_IDLE) {
                    final int edgesTouched = mInitialEdgesTouched[pointerId];
                    if ((edgesTouched & mTrackingEdges) != 0) {
                        mCallback.onEdgeTouched(edgesTouched & mTrackingEdges, pointerId);
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                for (int i = 0; i < pointerCount; i++) {
                    ...
                    final View toCapture = findTopChildUnder((int) x, (int) y);
                    final boolean pastSlop = toCapture != null && checkTouchSlop(toCapture, dx, dy, x, y);
                    if (pastSlop) {
            ...
            case MotionEvent.ACTION_MOVE: {
                if (mDragState == STATE_DRAGGING) {
                    ...
                    if (Math.abs(idx) > Math.abs(idy)) {
                        dragTo(mCapturedView.getLeft() + idx, mCapturedView.getTop(), idx, idy);
                    } else {
                        dragTo(mCapturedView.getLeft(), mCapturedView.getTop() + idy, idx, idy);
                    }
                    ...
                    for (int i = 0; i < pointerCount; i++) {
                        ...
                        final View toCapture = findTopChildUnder((int) x, (int) y);
                        if (checkTouchSlop(toCapture, dx, dy, x, y)
                                && tryCaptureViewForDrag(toCapture, pointerId)) {
                            break;
                        }
                    }
    ...
    // add by jun.sheng 解决侧滑过于灵敏问题 begin
    private boolean checkTouchSlop(View child, float dx, float dy, float x, float y) {
        boolean checkTouchSlop = checkTouchSlop(child, dx, dy);
        ...
        if (checkTouchSlop) {
            ...
            if (canScrollHorizontally(child, false, (int) dx, (int) x, (int) y)) {
                return false;
            }
        }

        return checkTouchSlop;
    }
    // add by jun.sheng 解决侧滑过于灵敏问题 begin
    ...
    private void releaseViewForPointerUp() {
        ...
        if (Math.abs(xvel) > Math.abs(yvel)) {
            dispatchViewReleased(xvel, 0);
        } else {
            dispatchViewReleased(0, yvel);
        }
    }
    ...
    private boolean isValidPointerForActionMove(int pointerId) {
        if (!isPointerDown(pointerId)) {
            Log.e(TAG, "Ignoring pointerId=" + pointerId + " because ACTION_DOWN was not received "
                    ...
                    + " BetterViewDragHelper did not receive all the events in the event stream.");
            return false;
        }
        return true;
    }
